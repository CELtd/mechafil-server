#!/usr/bin/env python3
"""Standalone simulation script for testing API responses against offline simulations.

This script runs the same simulation logic as the FastAPI server but offline,
...
def run_simulation(forecast_length_days=10*365, sector_duration_days=540, lock_target=0.3):
    """Run offline simulation with specified parameters."""
    ...
    # Save to JSON file
    output_file = "offline_simulation.json"
    with open(output_file, "w") as f:
        json.dump(results_to_save, f, indent=2)

    print(f"Offline simulation saved to {output_file}")
